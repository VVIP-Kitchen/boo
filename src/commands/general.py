...
class GeneralCommands(commands.Cog):
  ...
  async def generate_summary(self, ctx):
    """Summarize the last 15 minutes of channel chatter (fetched live from Discord)."""
    await ctx.defer()
    ...
    try:
      ...
      raw_messages: list[discord.Message] = [
        m
        async for m in ctx.channel.history(limit=200, after=cutoff, oldest_first=True)
      ...
      messages_text = "\n".join(
        f"**{m.author.name}:** {m.content}" for m in raw_messages
      )

      summary, _usage, _ = await to_thread(
        self.llm_service.chat_completions,
        prompt=(
          "Generate a snarky summary for the following Discord channel conversation: "
          f"{messages_text}"
        ),
        temperature=0.35,
        max_tokens=512,
      ...
      embed.add_field(
        ...
        value=f"{len(raw_messages)} messages from the last 15 minutes",
        inline=True,
      )
      embed.add_field(
        name="⏰ Time Range",
        value=(
          f"From {raw_messages[0].created_at.strftime('%Y-%m-%d %H:%M:%S')} "
          f"to {raw_messages[-1].created_at.strftime('%Y-%m-%d %H:%M:%S')} UTC"
        ),
        inline=False,
      )
